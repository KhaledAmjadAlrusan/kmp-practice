...
fun ListScreen(
    ...
    LaunchedEffect(Unit) {
        viewModel.dispatch(ListIntent.LoadMuseums)
    }
    when(state){
        is ListViewState.Content -> {
            val objects = (state as ListViewState.Content).museums
            if (objects.isEmpty()) {
        ...
        is ListViewState.Error -> EmptyScreenContent(Modifier.fillMaxSize())
        ListViewState.Loading -> LoadingScreen(Modifier.fillMaxSize())
    }
}
